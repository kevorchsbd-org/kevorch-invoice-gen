...
export const QuotationPDF: React.FC<DocumentProps> = ({ quotation }) => {
    ...
    const resolveClientLogo = async () => {
      if (client?.id) {
        try {
          ...
          console.warn('QuotationPDF logo load notice:', e);
        }
      }
  ...
  return (
    <div id="quotation-pdf-canvas" className="printable-area bg-white text-black p-8 max-w-[800px] mx-auto border border-gray-200 rounded-none shadow-none font-sans text-xs">
      ...
      <div className="grid grid-cols-2 gap-8 border-b border-gray-300 pb-6 mb-6">
        ...
        <div className="space-y-4">
          ...
          {/* 2. Left Metadata Block (Date, Valid Until, Payment Terms) */}
          <div className="space-y-1 text-black text-[11px]">
            <p><span className="font-bold">Quotation Date:</span> {quotationDate}</p>
            <p><span className="font-bold">Valid Until:</span> {validUntil}</p>
        ...
        <div className="space-y-4 text-right flex flex-col justify-between items-end">
          ...
          <div className="text-right text-black text-[11px] space-y-0.5 w-full">
            <p className="font-extrabold text-xs uppercase tracking-wider text-black">QUOTATION FOR</p>
      ...
      {/* Summary Block — Only Final Calculated Total Amount VALUE is BRAND RED (#E31B23) */}
      <div className="flex justify-end mb-6">
        <div className="w-64 bg-gray-50 border border-gray-300 rounded-none p-3 text-black">
          <div className="flex justify-between items-center text-sm font-extrabold">
            <span className="text-black font-bold">Total Amount:</span>
            <span className="text-[#E31B23] text-base font-black">₹{totalAmount.toLocaleString('en-IN')}</span>
          </div>
        </div>
      </div>
