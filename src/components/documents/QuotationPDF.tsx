...
export const QuotationPDF: React.FC<DocumentProps> = ({ quotation }) => {
  const { client, items, totalAmount, fromDetails, quotationNumber, quotationDate, validUntil, paymentTerms, notes, termsAndConditions } = quotation;

  return (
    <div id="quotation-pdf-canvas" className="printable-area bg-white text-gray-900 p-8 max-w-[800px] mx-auto border border-gray-200 rounded-none shadow-none font-sans text-xs">
      ...
      <div className="flex justify-between items-start border-b border-gray-200 pb-6 mb-6">
        ...
        <div className="text-right">
          ...
          <p className="font-extrabold text-sm text-gray-900">{quotationNumber}</p>
          <p className="text-gray-500 text-[11px]">Date: <span className="font-semibold text-gray-800">{quotationDate}</span></p>
          <p className="text-gray-500 text-[11px]">Valid Until: <span className="font-semibold text-gray-800">{validUntil}</span></p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6 bg-gray-50 p-4 rounded-lg border border-gray-100 mb-6">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-wider text-[#E31B23] mb-1">QUOTATION FOR:</p>
        ...
        <div className="border-l border-gray-200 pl-6 flex flex-col justify-between">
          <div>
            ...
            <p className="text-gray-800 text-[11px] font-medium leading-relaxed">{paymentTerms || "Standard terms apply"}</p>
          </div>
          {client.logoUrl && (
      ...
      <div className="flex justify-end mb-6">
        <div className="w-64 bg-gray-50 border border-gray-200 rounded-lg p-3 space-y-2">
          <div className="flex justify-between items-center text-sm font-extrabold border-t pt-2 border-gray-300">
            <span className="text-gray-900">Total Quotation Value:</span>
            <span className="text-[#E31B23] text-base">₹{totalAmount.toLocaleString('en-IN')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6 text-[10px] text-gray-600 border-t border-gray-200 pt-4 mb-8">
        <div>
          <p className="font-bold text-gray-900 text-xs mb-1 uppercase tracking-wider">NOTES / REMARKS:</p>
